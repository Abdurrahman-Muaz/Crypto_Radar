$(() => {
  ...
  const baseGridConfig = {
    columns: [
        ...
        cellTemplate: (cellElement, options) => {
          return $("<div>").dxButton({
            ...
            onClick: () => {
              ...
              toggleCoinsToFavorites(options.data.id, grid.option("gridId"));
      ...
      },
      {
        dataField: "name",
        caption: "Coin",
        cellTemplate: `<div class='d-flex gap-2'><a class='text-decoration-none text-body' href="detail/detail.html?coin={{data.id}}"><img src='{{data.image}}' style="height:1.5em"></img> {{value}} <span class='text-muted text-uppercase'>{{data.symbol}}</span></a></div>`,
      },
      {
      ...
      },
      {
        ...
        cellTemplate: (cellElement, options) => {
          const cellTemplate = $("<div>")
            ...
            .addClass(options.value >= 0 ? "text-success" : "text-danger");
    ...
    DevExpress.setTemplateEngine({
      compile: (_element) => {
        return Handlebars.compile(
          typeof _element == "string" ? _element : _element.get(0).outerHTML,
          {}
        );
      },
  ...
  const initCoinsTable = () => {
    ...
    cryptosGrid = $(`#${gridId}`).dxDataGrid(gridConfig).dxDataGrid("instance");
  };
